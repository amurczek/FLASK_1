...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///registrations.db"  # tworzę obiekt o nazwie db, połączony do flaska i do bazy danych
...
class Registration(db.Model):                               # w bazie danych ma być tabela Registration
    id = db.Column(db.Integer, primary_key=True)            # nazwa kolumny id- każdy ma inny, gdzie będą liczby całkowite typ Integer, klucz główny- bez powtórzeń
...
@app.route("/register", methods=["GET", "POST"])            # jeśli ktoś wejdzie na stronę /register   # GET i POST pobieranie danych  
def register():                                             # to uruchomi funkcję register
    if request.method == "POST":                            # jeśli formularz został wysłany             
        ...
        return "Dziękujemy za zapis na Bal Sylwestrowy! Twoja rezerwacja została zapisana!"  # potwierdzenie rejestracji

    return render_template("register.html")                 # wyświetla stronę rejestracji
